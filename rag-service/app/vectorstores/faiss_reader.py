...
class FaissReader:
    def __init__(self, storage_dir: Path) -> None:
        ...
        self._cache: dict[int, tuple[Any, list[dict[str, Any]]]] = {}
    ...
    def load_match_documents(self, match_id: int) -> list[dict[str, Any]]:
        _, rows = self._load_match(match_id)
    ...
    def search(
        ...
        *,
        ...
        index, rows = self._load_match(match_id)
        if not rows:
            return []
        ...
        for idx, score in zip(ids[0].tolist(), scores[0].tolist(), strict=True):
            ...
            doc = RetrievedDocument(
                ...
                matchId=int(payload.get("matchId")),
    ...
    def _load_match(self, match_id: int) -> tuple[Any, list[dict[str, Any]]]:
        cached = self._cache.get(match_id)
        if cached is not None:
            return cached

        match_dir = self._storage_dir / f"match_{match_id}"
        index_path = match_dir / "index.faiss"
        docs_path = match_dir / "documents.json"
        if not index_path.exists() or not docs_path.exists():
            raise NotFoundError(f"No FAISS index found for match {match_id}.")
        ...
        try:
            ...
            raise InternalServerError(f"Failed to load FAISS index for match {match_id}.") from exc

        try:
            with docs_path.open("r", encoding="utf-8") as f:
                rows = json.load(f)
        except json.JSONDecodeError as exc:
            raise InternalServerError(f"Malformed documents.json for match {match_id}.") from exc

        if not isinstance(rows, list):
            raise InternalServerError(f"Expected list in documents.json for match {match_id}.")

        cached_value = (index, rows)
        self._cache[match_id] = cached_value
        return cached_value
