...
from models import PressingRequest, PressingResponse, PlayerPressing


def _is_duel(event) -> bool:
    return event.type.primary in ("ground_duel", "aerial_duel")


def _is_won(event) -> bool:
    return "duel_won" in event.type.secondary


def _in_opponent_half(event) -> bool:
    return event.location.x > 50
...
def _build_insight(team_eff: float, first_eff: float, second_eff: float, drop: float) -> str:
    parts = []
...
def analyze_pressing(request: PressingRequest) -> PressingResponse:
    # Per-player stats split by half
    ...
    team_duels = {"1H": {"duels": 0, "won": 0}, "2H": {"duels": 0, "won": 0}}

    for event in request.events:
        if event.team.id != request.teamId:
            continue
        if not _is_duel(event):
            continue
        ...
        stats[pid]["position"] = event.player.position
        stats[pid][half]["duels"] += 1
        stats[pid]["opp_half"] += 1 if _in_opponent_half(event) else 0
    ...
    total_duels = team_duels["1H"]["duels"] + team_duels["2H"]["duels"]
    team_eff    = _efficiency(total_won, total_duels)
    ...
    top_presser = players[0].name if players else None
    ...
    return PressingResponse(
        ...
        intensityDrop=drop,
        insight=_build_insight(team_eff, first_eff, second_eff, drop),
        players=players,
