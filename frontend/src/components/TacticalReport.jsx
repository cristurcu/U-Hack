...
export default function TacticalReport({ dark = false, matchId }) {
  const [report, setReport] = useState(MOCK_REPORT);
  const [insight, setInsight] = useState(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    fetchTacticalReport(matchId).then(setReport).catch(() => {});
  }, [matchId]);

  const askInsight = async (topic) => {
    setLoading(true);
    try {
      const r = await fetchInsight(topic, matchId);
      setInsight(r);
    } finally {
      setLoading(false);
    }
  ...
  const s = report.summary || MOCK_REPORT.summary;
  const recommendations = report.recommendations || report.frontendOutput?.recommendations || report.topInsights || MOCK_REPORT.recommendations;
  const phases = report.phases || MOCK_REPORT.phases;
  const keyMoments = report.key_moments || report.keyMoments || MOCK_REPORT.key_moments;
