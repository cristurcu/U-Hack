...
import { fetchMatches, uploadPostMatchAnalysis } from "../lib/api";
...
export default function AnalysisMode({ liveState }) {
  ...
  const [upload, setUpload] = useState({ eventsFile: null, playersStatsFile: null });
  ...
  const refreshMatches = () => {
    ...
    return fetchMatches()
      .then((items) => {
        ...
        setMatches(items);
        setMatchId((current) => items.some((item) => item.id === current) ? current : items[0].id);
      })
      .catch(() => {})
  ...
  const submitUpload = async () => {
    if (!upload.eventsFile || !upload.playersStatsFile) {
      setUploadStatus("Select both JSON files.");
      return;
    }
    setUploadStatus("Running pipeline...");
    try {
      const result = await uploadPostMatchAnalysis(upload);
      setUploadStatus(`Analysed match ${result.matchId}.`);
      await refreshMatches();
      if (result.matchId) setMatchId(result.matchId);
    } catch (error) {
      setUploadStatus(error?.response?.data?.details || error?.message || "Upload failed.");
    }
  };
    ...
    <div style={{ display: "flex", height: "calc(100vh - 64px)", background: "#f5f5f5", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", gap: 8 }}>
          <div className="label" style={{ color: "rgba(255,255,255,0.4)", fontSize: 10 }}>Post-Match Upload</div>
          <input
            type="file"
            accept=".json,application/json"
            onChange={(e) => setUpload((u) => ({ ...u, eventsFile: e.target.files?.[0] || null }))}
            style={{ color: "rgba(255,255,255,0.55)", fontSize: 10 }}
          />
          <input
            type="file"
            accept=".json,application/json"
            onChange={(e) => setUpload((u) => ({ ...u, playersStatsFile: e.target.files?.[0] || null }))}
            style={{ color: "rgba(255,255,255,0.55)", fontSize: 10 }}
          />
          <input
            placeholder="Match ID (optional)"
            onChange={(e) => setUpload((u) => ({ ...u, matchId: e.target.value }))}
            style={{ fontSize: 11, padding: "6px 8px", background: "#111", color: "#fff", border: "1px solid rgba(255,255,255,0.12)" }}
          />
          <button
            onClick={submitUpload}
            style={{ padding: "7px 10px", border: "none", background: "var(--uc-red)", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}
          >
            RUN PIPELINE
          </button>
          {uploadStatus && (
            <div className="font-mono" style={{ color: "rgba(255,255,255,0.45)", fontSize: 10, lineHeight: 1.4, whiteSpace: "normal" }}>{uploadStatus}</div>
          )}
        </div>
      ...
      <div style={{ flex: 1, padding: "32px 40px", overflow: "auto", minWidth: 0 }}>
        ...
        <div className="fade-up">
          ...
          {tab === "decision" && <DecisionQualityReport matchId={matchId} events={liveState?.decision_events?.length ? liveState.decision_events : undefined} />}
