...
import { fetchDecisionQualityEvents } from "../lib/api";
...
export default function DecisionQualityReport({ events, matchId }) {
  const [remoteEvents, setRemoteEvents] = useState(null);

  useEffect(() => {
    if (!matchId) return;
    let active = true;
    fetchDecisionQualityEvents(matchId)
      .then((rows) => { if (active && rows.length) setRemoteEvents(rows); })
      .catch(() => {});
    return () => { active = false; };
  }, [matchId]);

  const resolvedEvents = events || remoteEvents || MOCK_EVENTS;
  ...
  const recentEvents = [...resolvedEvents].reverse().slice(0, 6);
